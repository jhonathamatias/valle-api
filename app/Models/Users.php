...
class Users
{
    ...
    public function search(array $params = []): array
    {
        ...
        $users = $usersRepository->search($params, 'id, name, email, created_at');

        if ($users['total'] === 0) {
            throw new NotFoundException('Não foi encontrado nenhum usúario');
        }

        return Mapper::collectionMap('users', $users['data'], $users['total']);
    }
